...
export default function AuthModal({ isOpen, onClose, initialMode = 'signin' }) {
  ...
  async function handleAutoVerifyOtp() {
    setLoading(true)
  ...
  // Manual verification function (with event parameter for form submission)
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 px-4">
      ...
      <div className="bg-white border-2 border-black p-6 w-full max-w-md relative z-10 rounded-xl">
        ...
        {message && (
          <div className={`mb-4 p-3 text-sm border-2 border-black ${message.includes('complete') || message.includes('successful') || message.includes('Check your email')
            ? 'bg-black text-white'
            : 'bg-white text-black'
            }`}>
            {message}
