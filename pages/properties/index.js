...
export default function AllProperties() {
  ...
  async function loadProperties() {
    setLoading(true)
    // REMOVED .eq('status', 'available') to show occupied/not available properties as requested
    ...
    // Assuming "Most Favorite" means properties with a significant number of favorites (e.g. >= 1 or specific threshold)
    if (filterMostFavorite) {
       filteredData = filteredData.filter(property => {
        const stats = propertyStats[property.id] || { favorite_count: 0 }
        return stats.favorite_count >= 1 // You can adjust this threshold
      })
    }
  ...
  const renderPropertyCard = (property) => {
    ...
    // Logic for "Tenants Favorite" Badge (e.g., more than 2 favorites)
        ...
        >
          ...
          <div className="relative aspect-[4/3] overflow-hidden bg-gray-100">
            ...
            <div className="absolute top-3 left-3 z-10 flex flex-col gap-1 items-start">
              ...
              {isTenantsFavorite && (
                 <span className="px-2 py-0.5 text-[10px] uppercase font-bold tracking-wider rounded-md shadow-sm backdrop-blur-md bg-rose-500 text-white flex items-center gap-1">
          ...
          <div className="p-4 flex-1 flex flex-col">
            <div className="mb-2">
                ...
                {/* Total Count Stars / Ratings Section */}
                <div className="flex items-center gap-2 mb-3 bg-gray-50 p-1.5 rounded-lg w-fit">
                    <div className="flex items-center gap-0.5">
                        <svg className="w-4 h-4 text-yellow-400 fill-yellow-400" viewBox="0 0 24 24">
                          <path d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                        </svg>
                        <span className="font-bold text-gray-900 text-sm">{stats.avg_rating > 0 ? stats.avg_rating.toFixed(1) : ''}</span>
                    </div>
                    {stats.review_count > 0 ? (
                       <span className="text-[10px] text-gray-500 font-medium">({stats.review_count} Reviews)</span>
                    ) : (
                       <span className="text-[10px] text-gray-400 font-medium">No reviews</span>
                    )}
                </div>
            </div>
    ...
    <div className="min-h-screen bg-[#FAFAFA] font-sans text-black flex flex-col">
      <div className="max-w-[1500px] mx-auto w-full px-4 sm:px-6 py-6 flex-1">
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <aside className="w-full lg:w-64 shrink-0 space-y-6 lg:sticky lg:top-24 bg-white p-5 rounded-2xl border border-gray-100 shadow-sm h-fit">
