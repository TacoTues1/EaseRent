...
export async function sendOTP(phoneNumber, code) {
  const message = `RentSphere: Your verification code is ${code}. Valid for 10 minutes. Do not share this code.`;
...
export async function sendBookingConfirmation(phoneNumber, booking) {
  const message = `RentSphere : Your viewing for "${booking.propertyName}" on ${booking.date} at ${booking.time} is CONFIRMED. Ref: ${booking.id}`;
...
export async function sendBillNotification(phoneNumber, bill) {
  const message = `RentSphere: You received a bill for ${bill.propertyName}. Amount: ₱${bill.amount}. Due: ${bill.dueDate}. Log in to pay.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendBookingReminder(phoneNumber, booking) {
  const message = `RentSphere Reminder: Your viewing for "${booking.propertyName}" is in 12 hours (${booking.date} @ ${booking.time}). Please be on time.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendUnreadMessageNotification(phoneNumber, unreadCount, senderName) {
  const message = `RentSphere: You have ${unreadCount} unread message(s) from ${senderName} pending for 6+ hours. Log in to reply.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendNewApplicationNotification(phoneNumber, application) {
  const message = `RentSphere Alert: New application received from ${application.applicantName} for "${application.propertyName}". Log in to review.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendNewBookingNotification(phoneNumber, booking) {
  const message = `RentSphere Alert: New viewing request from ${booking.tenantName} for "${booking.propertyName}" on ${booking.date} at ${booking.time}.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendPaymentReminder(phoneNumber, payment) {
  const message = `RentSphere Reminder: Payment of ₱${payment.amount} for ${payment.propertyName} is due on ${payment.dueDate}.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendMaintenanceUpdate(phoneNumber, maintenance) {
  const message = `RentSphere: Maintenance request "${maintenance.title}" updated to: ${maintenance.status}. ${maintenance.note || ''}`.trim();
  return sendSMS(phoneNumber, message);
}
...
export async function sendApplicationStatus(phoneNumber, application) {
  ...
  const message = `RentSphere: ${statusMessages[application.status] || 'Application status updated.'}`;
  return sendSMS(phoneNumber, message);
}
