...
/**
 * Send Application Status Update (For Tenant)
 */
export async function sendApplicationStatus(phoneNumber, application) {
  const statusLabel = application.status === 'approved' ? 'APPROVED' : application.status === 'rejected' ? 'REJECTED' : application.status?.toUpperCase() || 'UPDATED';
  const message = `Abalay: Your application for "${application.propertyName}" has been ${statusLabel}. Log in for details.`;
  return sendSMS(phoneNumber, message);
}

/**
 * Send Renewal Status SMS (For Tenant)
 */
export async function sendRenewalStatus(phoneNumber, { propertyTitle, status, newEndDate, signingDate }) {
  const statusLabel = status === 'approved' ? 'APPROVED' : status === 'rejected' ? 'REJECTED' : status?.toUpperCase() || 'UPDATED';
  let message = `Abalay: Your renewal request for "${propertyTitle}" has been ${statusLabel}.`;
  if (newEndDate) message += ` New end date: ${newEndDate}.`;
  if (signingDate) message += ` Signing date: ${signingDate}.`;
  message += ' Log in for details.';
  return sendSMS(phoneNumber, message);
}

/**
 * Send Renewal Request SMS (For Landlord)
 */
export async function sendRenewalRequest(phoneNumber, { tenantName, propertyTitle, proposedDate }) {
  const message = `Abalay: ${tenantName} has requested a contract renewal for "${propertyTitle}". Proposed date: ${proposedDate}. Log in to review.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendNewBookingNotification(phoneNumber, booking) {
  const message = `Abalay: New viewing request from ${booking.tenantName} for "${booking.propertyName}" on ${booking.date} at ${booking.time}.`;
  return sendSMS(phoneNumber, message);
}
