...
export async function sendOTP(phoneNumber, code) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝: Your verification code is ${code}. Valid for 10 minutes. Do not share this code.`;
...
export async function sendBookingConfirmation(phoneNumber, booking) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝 : Your viewing for "${booking.propertyName}" on ${booking.date} at ${booking.time} is CONFIRMED. Ref: ${booking.id}`;
...
export async function sendBillNotification(phoneNumber, bill) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝: You received a bill for ${bill.propertyName}. Amount: ₱${bill.amount}. Due: ${bill.dueDate}. Log in to pay.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendBookingReminder(phoneNumber, booking) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝 Reminder: Your viewing for "${booking.propertyName}" is in 12 hours (${booking.date} @ ${booking.time}). Please be on time.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendUnreadMessageNotification(phoneNumber, unreadCount, senderName) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝: You have ${unreadCount} unread message(s) from ${senderName} pending for 6+ hours. Log in to reply.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendNewApplicationNotification(phoneNumber, application) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝 Alert: New application received from ${application.applicantName} for "${application.propertyName}". Log in to review.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendNewBookingNotification(phoneNumber, booking) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝 Alert: New viewing request from ${booking.tenantName} for "${booking.propertyName}" on ${booking.date} at ${booking.time}.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendMoveInNotification(phoneNumber, moveIn) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝: Welcome home! You've been assigned to "${moveIn.propertyName}". Contract: ${moveIn.startDate} to ${moveIn.endDate}. Rent: ₱${moveIn.rentAmount}. Log in for details.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendContractNearingEnd(phoneNumber, contract) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝 Reminder: Your contract for "${contract.propertyName}" ends on ${contract.endDate} (${contract.daysRemaining} days left). Contact your landlord to renew or extend.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendEndContractNotification(phoneNumber, info) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝: Your contract for "${info.propertyName}" has been ENDED. Reason: ${info.reason || 'Approved move-out'}. Please ensure all dues are cleared.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendPaymentReminder(phoneNumber, payment) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝 Reminder: Payment of ₱${payment.amount} for ${payment.propertyName} is due on ${payment.dueDate}.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendMaintenanceUpdate(phoneNumber, maintenance) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝: Maintenance request "${maintenance.title}" updated to: ${maintenance.status}. ${maintenance.note || ''}`.trim();
  return sendSMS(phoneNumber, message);
}
...
export async function sendMaintenanceDoneNotification(phoneNumber, request) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝 Alert: Tenant ${request.tenantName} marked maintenance "${request.title}" as DONE. Please review and log the cost to officially complete it.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendApplicationStatus(phoneNumber, application) {
  ...
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝: ${statusMessages[application.status] || 'Application status updated.'}`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendRenewalStatus(phoneNumber, renewal) {
  ...
  if (renewal.status === 'approved') {
    message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝: Renewal for "${renewal.propertyTitle}" APPROVED! New End: ${renewal.newEndDate}. Signing: ${renewal.signingDate}.`;
  } else {
    message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝: Renewal request for "${renewal.propertyTitle}" was REJECTED. Contact landlord for details.`;
  }
  return sendSMS(phoneNumber, message);
}
...
export async function sendRenewalRequest(phoneNumber, request) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝: Renewal request for "${request.propertyTitle}" from ${request.tenantName}. Proposed date: ${request.proposedDate}. Pls check dashboard.`;
  return sendSMS(phoneNumber, message);
}
...
export async function sendFamilyMemberAddedSMS(phoneNumber, { tenantName, memberName, propertyTitle }) {
  const message = `𝐓𝐞𝐬𝐬𝐲𝐍𝐓𝐞𝐝: ${tenantName} added a family member (${memberName}) to "${propertyTitle}". Family members can pay bills & submit maintenance. Check dashboard.`;
  return sendSMS(phoneNumber, message);
}
